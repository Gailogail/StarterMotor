...
api = Api(
    ...
    version="1.0",
    title="My First API",
    description="Here's an API to try, there are several methods here you can ping",
)


@api.route("/sum")
@api.doc(params={"first": "2", "second": "3"})
class Sum(Resource):
    def get(self):
        return int(request.args.get("first")) + int(request.args.get("second"))
